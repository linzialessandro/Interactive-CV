import { ProfileData } from '../types';

export const profileData: ProfileData = {
  name: 'Alessandro Linzi',
  title: 'Mathematician | Research Scientist',
  summary: 'Mathematician specializing in hypercompositional algebra, model theory, and valuation theory with experience in international research environments. Published author with 9 peer-reviewed publications in mathematical journals. Interested in the intersection of pure mathematics and computational applications, including AI logical reasoning and proof assistants. Seeking research opportunities that combine theoretical mathematics with practical implementation.',
  experience: [
    {
      title: 'Mathematics Teacher',
      company: 'ISIS E. Mattei di Latisana, Italy',
      period: '2025',
      responsibilities: [
        'Teaching mathematics to 6 classes across high school years 1-4.',
        'Maintaining documentation systems and curriculum planning.',
      ],
    },
    {
      title: 'Assistant Professor (Postdoc)',
      company: 'University of Nova Gorica, Slovenia',
      period: '2022 - 2024',
      responsibilities: [
        'Conducted research resulting in 8 peer-reviewed publications and 5 submissions.',
        'Achieved habilitation as Assistant in Mathematics in Slovenia.',
        'Served as editor and peer reviewer for mathematical journals.',
        'Co-authored textbook: “Mathematics for Engineering and Management” (ISBN: 978-961-7025-41-5).',
      ],
    },
    {
      title: 'PhD Researcher & Teaching Assistant',
      company: 'University of Szczecin, Poland',
      period: '2018 - 2022',
      responsibilities: [
        'Published 2 peer-reviewed papers during doctoral studies.',
        'Taught undergraduate course “Information Technologies" for School of Humanities.',
        'Assisted in mentoring master\'s students.',
        'Participated in international valuation theory research group.',
      ],
    },
     {
      title: 'Teaching Assistant & Tutor',
      company: 'Universities of Florence & Udine, Italy',
      period: '2016 - 2018',
      responsibilities: [
        'Mathematics tutoring for School of Agriculture students.',
        'Developed teaching materials for diverse student backgrounds.',
      ],
    },
  ],
  education: [
    {
      degree: 'PhD in Mathematics',
      institution: 'University of Szczecin, Poland',
      period: '2018 - 2022',
      details: 'Dissertation: “Algebraic hyperstructures in the model theory of valued fields"',
    },
    {
      degree: 'Master\'s in Mathematics (Didactical Curriculum), 110/110 Cum Laude',
      institution: 'University of Florence, Italy',
      period: '2016 - 2018',
      details: 'Thesis: "The Theorem of Ax-Kochen-Ershov"',
    },
    {
      degree: 'Bachelor\'s in Mathematics, 101/110',
      institution: 'University of Udine, Italy',
      period: '2010 - 2015',
      details: 'Thesis: "The Consistency of Arithmetics"',
    },
    {
      degree: 'International Exchange Program',
      institution: 'AFS Costa Rica',
      period: '2008 - 2009',
      details: '6-month intercultural immersion program during high school.',
    },
  ],
  publications: [
      {
          title: 'On the hyperfields associated to valued fields',
          outlet: 'Journal of Pure and Applied Algebra',
          date: '2025',
      },
      {
          title: 'Preface to the Special Issue “Algebraic Structures and Graph Theory, 2nd Edition”',
          outlet: 'Mathematics, 13(4):577',
          date: '2025',
      },
      {
          title: 'Notes on valuation theory for Krasner hyperfields',
          outlet: 'Isr. J. Math.',
          date: '2025',
      },
      {
          title: 'Polygroup objects in regular categories',
          outlet: 'AIMS Mathematics, 9(5)',
          date: '2024',
      },
      {
          title: 'A result of Krasner in categorial form',
          outlet: 'Mathematics, 11(24): 4923',
          date: '2023',
      },
      {
          title: 'Dependence relations and grade fuzzy set',
          outlet: 'Symmetry, 15(2):311',
          date: '2023',
      },
      {
          title: 'Characteristic, C-characteristic and positive cones in hyperfields',
          outlet: 'Mathematics, 11(3): 779',
          date: '2023',
      },
      {
          title: 'Orderings and valuations in hyperfields',
          outlet: 'J. of Algebra, Vol. 611, pp. 399-421',
          date: '2022',
      },
      {
          title: 'Oettli-Théra and Caristi-Kirk ball spaces and applications',
          outlet: 'J. of fixed point theory and applications, Vol. 21, No. 98',
          date: '2019',
      },
  ],
  skills: {
    'Mathematical Areas': ['Model Theory', 'Valued Fields', 'Hypercompositional Algebra', 'Mathematical Logic', 'Category Theory'],
    'Programming': ['Python', 'LaTeX', 'SQL', 'HTML/CSS', 'Git'],
    'Mathematical Software': ['Agda', 'Lean', 'Coq', 'MATLAB', 'Sage', 'Excel'],
    'AI/ML Familiarity': ['Experience with language models', 'Interest in automated theorem proving'],
    'Languages': ['Italian (Native)', 'English (Fluent)', 'Spanish (Fluent)', 'Slovenian (Basic)', 'Polish (Basic)'],
  },
  contact: {
    email: 'alessandro.linzi.phd@icloud.com',
    linkedin: 'https://www.linkedin.com/in/alessandrolinzi',
    github: 'https://github.com/linzialessandro',
    orcid: 'https://orcid.org/0000-0002-1422-7674',
    cvUrl: 'https://sites.google.com/view/alessandro-linzi/cv',
  },
};